...
async def rejoin(bot, m, s):
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        for f in s["files"]:
            ...
            await bot.download_media(f["id"], p)
            parts.append(p)
        ...
        subprocess.run(["7z", "x"] + parts + [f"-o{out}", "-y"])
        await upload_folder(bot, m, out)
